...
test('presents the three optional keepsake products', () => {
  ...
  assert.match(addOnProductsSection, /Product Add-ons/);
  ...
  assert.match(addOnProductsSection, /albumVideoSlides[\s\S]*albumPhotoSlides/);
  assert.match(addOnProductsSection, /album-media-grid[\s\S]*albumVideoSlides[\s\S]*albumPhotoSlides/);
  ...
  assert.match(addOnProductsSection, /<strong>From \$\d+<\/strong>/);
  ...
  assert.match(addOnProductsSection, /className="add-on-product-card__info"[\s\S]*Choose from our available bag styles[\s\S]*ProductCarousel slides=\{bagSlides\}/);
  assert.match(styles, /\.add-on-product-card__info[\s\S]*text-align:\s*right/);
  ...
  assert.match(styles, /\.product-carousel--album-video \.product-carousel__slide[\s\S]*width:\s*240px[\s\S]*height:\s*320px/);
  assert.match(styles, /\.product-carousel--album-photos \.product-carousel__slide[\s\S]*width:\s*320px[\s\S]*height:\s*320px/);
  assert.match(addOnProductsSection, /product-carousel--album-video/);
  assert.match(addOnProductsSection, /product-carousel--album-photos/);
...
test('describes the custom round badge offering', () => {
  for (const detail of ['Custom Round Shape Badges', 'fridge.png', 'keychain.png', 'Custom Fridge Magnet', 'Custom Keychain', '32 mm, 44 mm, 58 mm, or 75 mm', 'From just 1 piece', 'Full-colour HD', 'scratch-resistant', 'built-in pin back', 'Design tips', 'How to order']) {
